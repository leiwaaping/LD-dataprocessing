...
def main(argv):
   ...
   emptysnps=compro(targetfile,inputfile)
   cout_candidate_loci(LDmaxLen,outputfile)
   #show runtime
   endtime = datetime.datetime.now()
   time = "Run time is %s s\n"%(endtime - starttime).seconds
   print (time)
   for emptysnp in emptysnps:
       print ("Can't find "+emptysnp+"!")
    ...
    for snp in snps:
        ...
        for vcfs in vcfsall:# readin each file
            for vcf in vcfs:# readin each line
                ...
                if matchObj:
                    snpstr = snp + '\t' + re.sub(r'(?i)chr','',matchObj.group(1)) + '\t' + matchObj.group(2) + '\t' + matchObj.group(3) + '\t'
                    ref = matchObj.group(3)+'\t'
        if snpstr == "":
            emptysnps.append(snp)
        else:
            out += '\n' + snpstr
            for vcfs in vcfsall:
                vcfnum1 = ""
